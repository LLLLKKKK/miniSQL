...
bool FixedSizeChunkAllocator::init(uint32_t requestSize, uint32_t maxRequestChunk) {
    _maxRequestChunk = maxRequestChunk;
    _fixedSize = requestSize > sizeof(void *) ? requestSize : sizeof(void *);
    void* buf;
    if (int err = posix_memalign(&buf, _fixedSize, _fixedSize * maxRequestChunk) != 0) {
        MINISQL_LOG_ERROR("Allocate memory %u failed! %s",
                          _fixedSize * maxRequestChunk, strerror(err));
        return false;
    }
    _buffer = static_cast<char*>(buf);
    _end = _buffer + _fixedSize * maxRequestChunk;
    _free = _buffer;
    return true;
}
